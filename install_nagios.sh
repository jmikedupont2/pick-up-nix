...
for x in v3-0 v3-4 v3-3 v3-2 v3-1 v2-4 v2-3 v2-2 v2-1 v2-0
 do
    ...
    ssha $x "cd /mnt/data1/nix/time/2024/03/26/pick-up-nix && git fetch && git checkout origin/master &&git checkout master && git pull origin master"
 #    ssha $x "sudo apt update && sudo apt install -y nagios-nrpe-server  "
    ...
    ssha $x "sudo systemctl restart  nagios-nrpe-server  "

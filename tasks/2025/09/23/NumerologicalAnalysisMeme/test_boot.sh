...
# test_boot.sh for NumerologicalAnalysisMeme experiment
...
echo "Running boot.sh for NumerologicalAnalysisMeme experiment and capturing output to ${LOG_FILE}"

# Enter nix develop shell and run boot.sh, capturing all output
nix develop "${EXPERIMENT_DIR}" --command bash -c "
    echo 'Entering nix develop shell for NumerologicalAnalysisMeme experiment...'

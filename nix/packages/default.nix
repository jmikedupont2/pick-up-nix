{ commonPackages = pkgs: geminiCliSrc: { packages = { hello = pkgs.hello; figlet = pkgs.figlet; which = pkgs.which; "gemini-cli" = pkgs.callPackage ./pkgs/gemini-cli { inherit geminiCliSrc; }; batch-task-processor = pkgs.writeShellApplication { name = "batch-task-processor"; runtimeInputs = [ pkgs.bash ]; text = ''#!/usr/bin/env bash
